...
test('InfoPackage in get should validate the package name', assert => {
...
test('InfoPackage in get should obtain name', assert => {
  ...
  assert.deepEqual(data.version, 'last')
  assert.end()
})

test('InfoPackage in get should obtain name and scope', assert => {
...
test('InfoPackage in get should obtain name, scope and tag', assert => {
...
test('InfoPackage in get should obtain name, scope and version', assert => {
...
test('InfoPackage in get should obtain name and tag', assert => {
  const data = info.get('package-name@last')
  assert.deepEqual(data.valid, true)
  assert.deepEqual(data.name, 'package-name')
  assert.deepEqual(data.escapedName, 'package-name')
  assert.deepEqual(data.scope, null)
...
test('InfoPackage in get should obtain name and version', assert => {
